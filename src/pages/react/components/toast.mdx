...
`useToast()` returns a function `(options) => void`. Options: `title`, `text`, `color`, `icon`, `iconProps`, `closeButton`, `onClosed` — same shape as the matching `Toast` props. The hook is a thin wrapper over the portal queue mounted by [`UIProvider`](/react/components/ui-provider/).
