...
`color` accepts any of the cladd accent tokens — the accent drives the selected-state fill. When unset, the radio inherits the theme accent from [`UIProvider`](/react/components/ui-provider/), so a brand-coloured radio set doesn't usually need a `color` prop. Pass one when the selection signals severity (`red` for a destructive option) or sits alongside a non-default control that needs to read as a single unit.
