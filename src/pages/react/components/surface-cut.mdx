...
A `SurfaceCut` renders three layers, in this order:
...
2. **Background layer** — an absolutely-positioned `div` sized to the root. Paints the recessed fill (`bg-cladd-surface-cut`), the inset outline ring (when `outline`), and the hover/pressed overlays. There's no `bgClassName` here — the cut's fill is fixed, by design.
3. **Content wrapper** — a `relative h-full` `div` (`SurfaceCutContent`) that wraps `children`. The `relative` positioning lifts the content above the absolute bg layer so the children render on top of the recessed fill. It has no padding or layout of its own — that's `contentClassName`'s job.
