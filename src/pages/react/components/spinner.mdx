...
export default ({ children }) => (
  <DocsLayout
    title="Spinner — Cladd"
    description="Accent-aware spinning ring for inline loading states — sizes 2xs → 2xl, eleven accent colors, drops into Button, Chip, Input, and Toast without extra wiring."
  >
    {children}
  </DocsLayout>
);
...
`color` accepts any of the eleven cladd accent tokens — the ring takes the accent's primary tone. By default the spinner inherits the theme accent set on [`UIProvider`](/react/components/ui-provider/), so a brand spinner doesn't usually need a `color` prop at all. Pass one when you want to signal severity (`red` for failing work, `yellow` for a warning state) or match the spinner to a non-default control like a green "deploying" pill.
